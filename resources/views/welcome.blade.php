        ...
        <div class="px-10 py-20 relative overflow-hidden sm:px-5 xl:mt-20">
            ...
            <div class="grid grid-cols-4 gap-10 sm:mt-10 sm:flex sm:flex-col sm:gap-5 lg:flex-row ml-20">
                ...
                <a href="{{ route('welcome', ['event_id' => $e->id])}}">
                    <div class="px-5 py-2 pb-10 shadow-lg rounded-md bg-white h-fullcursor-pointer transform transition-transform duration-300 hover:scale-105">
                        <img src="{{ asset($e->image) }}" alt="{{ $e->name }}" class="rounded-md">
                        <div class="flex flex-row mt-5 items-center">
                            <h1 class="text-xl font-bold w-1/2">{{ $e->name }}</h1>
                            <p class="text-sm text-gray-400 w-1/2">{{ $e->date }}</p>
                        </div>
